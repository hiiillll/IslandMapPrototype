...
// Upgrade NOTE: replaced 'mul(UNITY_MATRIX_MVP,*)' with 'UnityObjectToClipPos(*)'
	...
	half4 LightingSuimonoLight (SurfaceOutput s, half3 lightDir, half3 viewDir, half atten){
		...
		if (suimono_trans_on == 1.0){
			...
			//bright
			col.rgb = col.rgb + (((fixed3(ndf,ndf,ndf) * fixed3(h,h,h) * lerp(fixed3(1,1,1), fixed3(0.1,0.1,0.1), _roughness)) * atten) * outLight.rgb * _SpecularColor.rgb)*1.75 * _specularPower;
		}
		...
		// Keep both native SUIMONO surfaces on the same authored deep-ocean
		// palette in standalone builds, where the legacy screen buffers can
		// otherwise resolve to the flat cyan fallback color.
		half normalRipple = saturate((abs(s.Normal.x) + abs(s.Normal.y)) * 1.65h);
		half waveHighlight = saturate((h - 0.16h) * 1.25h);
		waveHighlight = waveHighlight * waveHighlight;
		fixed3 deepOcean = fixed3(0.008h, 0.032h, 0.14h);
		fixed3 waveCrest = fixed3(0.025h, 0.15h, 0.38h);
		fixed3 authoredOcean = lerp(deepOcean, waveCrest, normalRipple * 0.32h);
		authoredOcean += _LightColor0.rgb * waveHighlight * 0.09h;
		col.rgb = lerp(col.rgb, authoredOcean, 0.9h);
